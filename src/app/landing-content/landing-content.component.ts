    ...
    */
    if (this.userId > 0)
      this.router.navigate(['home']);
            ...
            this.getDataFromServer('getThumbnails', 'appApiCode2.php', []);
    }
  }
  override postSuccessApi(file: string, responseJson: any) {
    this.topMen = [];
    this.topWomen = [];
    if (responseJson.action == "getThumbnails") {
      console.log(responseJson);
      responseJson.topMen.forEach((element: any) => {
        this.topMen.push(new User(element));
      });
      responseJson.topWomen.forEach((element: any) => {
        this.topWomen.push(new User(element));
      });
      console.log('this.topWomen', this.topWomen);
    }
  }
